...
import { ThumbsDown, ThumbsUp, SlidersHorizontal } from 'lucide-react';
...
type ScoreKey = 'experience' | 'speed' | 'investigators' | 'competitor' | 'relevance' | 'stability' | 'sponsorFit';
...
const labels: Record<Locale, any> = {
  en: {
    ...
    subtitle:
      'Add CTM feedback loops to ranking logic. Compare before-vs-after ranking, inspect learned weights, and keep an auditable feedback timeline.',
    filters: 'Base Filter Context',
    rankingBefore: 'Before (Base Policy)',
    rankingAfter: 'After (Learned Policy)',
    compare: 'Rank Shift',
  ...
  zh: {
    title: 'Site Feasibility（人类反馈强化版）',
    subtitle: '在站点排序中加入 CTM 反馈回路，比较前后排名变化、观察权重学习过程，并保留可审计日志。',
    filters: '基础筛选上下文',
    rankingBefore: '反馈前（基础策略）',
    rankingAfter: '反馈后（学习策略）',
    compare: '排名变化',
    profile: '反馈控制台',
    weights: '学习后权重',
    log: '反馈日志（审计）',
  ...
  de: {
    title: 'Site Feasibility mit Human Feedback',
    subtitle:
      'CTM-Feedback in das Ranking integrieren: Before-vs-after, gelernte Gewichte und auditierbares Feedback-Protokoll.',
    filters: 'Basis-Filterkontext',
    rankingBefore: 'Vorher (Basis-Policy)',
    rankingAfter: 'Nachher (gelernte Policy)',
    compare: 'Rangverschiebung',
    profile: 'Feedback-Steuerung',
    weights: 'Gelernte Gewichte',
    log: 'Feedback-Log (Audit)',
    like: 'Aufwerten',
    dislike: 'Abwerten',
    apply: 'Feedback anwenden',
...
function rankSites(sites: SiteRecord[], filters: any, weights: Weights): RankedSite[] {
  const rows = sites
    .map((site) => {
      ...
      const experienceRaw = matched.reduce((sum, p) => sum + p.ctgovTrials, 0);
      const competitorRaw = matched.reduce((sum, p) => sum + p.competitorTrials, 0);
...
export function SiteFeasibilityFeedbackPrototype({ locale, sites }: { locale: Locale; sites: SiteRecord[] }) {
  ...
  const therapeuticAreas = useMemo(() => ['All', ...Array.from(new Set(sites.flatMap((s) => s.conditionProfiles.map((p) => p.therapeuticArea))))], [sites]);
  const indications = useMemo(() => ['All', ...Array.from(new Set(sites.flatMap((s) => s.conditionProfiles.filter((p) => filters.area === 'All' || p.therapeuticArea === filters.area).map((p) => p.indication))))], [sites, filters.area]);
  const countries = useMemo(() => ['All', ...Array.from(new Set(sites.map((s) => s.country)))], [sites]);

  const before = useMemo(() => rankSites(sites, filters, baseWeights), [sites, filters]);
  ...
  function applyFeedback() {
    if (!chosenSite || reasons.length === 0) return;

    const beforeWeights = { ...weights };
    const delta = 0.012 * strength;
    const updated = { ...weights };

    for (const key of reasons) {
      updated[key] = Math.max(0.05, action === 'like' ? updated[key] + delta : updated[key] - delta);
    }
    ...
    <div className="space-y-8">
      ...
      <section className="noise-border rounded-lg p-4">
        ...
        <div className="grid gap-3 md:grid-cols-5">
          ...
          <label className="text-sm">Indication
          ...
          <label className="text-sm">Phase
            <select className="mt-1 w-full rounded border border-ink/20 bg-transparent px-2 py-2" value={filters.phase} onChange={(e) => setFilters({ ...filters, phase: e.target.value })}>
              <option>All</option><option>Phase 2</option><option>Phase 3</option><option>Phase 4</option>
            </select>
          </label>
          ...
          <label className="text-sm">{t.sponsor}
      ...
      <section className="grid gap-4 xl:grid-cols-3">
        <article className="noise-border rounded-lg p-4 xl:col-span-1">
          ...
          <div className="mt-3 flex gap-2">
            <button type="button" onClick={() => setAction('like')} className={`scanline rounded border px-3 py-2 text-sm ${action==='like'?'border-accent1':'border-ink/20'}`}><ThumbsUp size={14} className="mr-1 inline" />{t.like}</button>
            <button type="button" onClick={() => setAction('dislike')} className={`scanline rounded border px-3 py-2 text-sm ${action==='dislike'?'border-red-500':'border-ink/20'}`}><ThumbsDown size={14} className="mr-1 inline" />{t.dislike}</button>
          </div>
          ...
          <label className="mt-3 block text-sm">{t.strength}: {strength}
            <input type="range" min={1} max={5} value={strength} onChange={(e) => setStrength(Number(e.target.value))} className="mt-1 w-full" />
          </label>
          <button type="button" onClick={applyFeedback} className="scanline mt-3 rounded border border-ink/20 px-3 py-2 text-sm">{t.apply}</button>
          ...
          <div className="mt-4">
            ...
            {(Object.keys(weights) as ScoreKey[]).map((k) => (
              <div key={k} className="mb-2">
                ...
                <div className="h-2 rounded bg-ink/10"><div className="h-full rounded bg-accent1" style={{ width: `${weights[k] * 100}%` }} /></div>
              </div>
            ))}
          </div>
        </article>
          ...
          <div className="space-y-2 text-sm">
            {before.slice(0, 8).map((row, idx) => (
              <div key={row.site.id} className="rounded border border-ink/15 p-2">
                <p className="font-medium">#{idx + 1} {row.site.name}</p>
      ...
      <section className="noise-border rounded-lg p-4">
        ...
        {log.length === 0 ? <p className="text-sm text-ink/65">No feedback yet.</p> : null}
        <ol className="space-y-3 border-l border-ink/20 pl-4">
          {log.map((entry, idx) => (
            <li key={`${entry.timestamp}-${idx}`} className="relative text-sm">
              <span className="absolute -left-[1.15rem] top-1 h-2 w-2 rounded-full bg-accent3" />
              <p className="font-mono text-[11px] uppercase tracking-[0.12em] text-ink/60">{new Date(entry.timestamp).toLocaleString()}</p>
              <p className="mt-1">{entry.action.toUpperCase()} · {entry.siteName} · {entry.reasons.map((r) => t.reasons[r]).join(', ')}</p>
            </li>
          ))}
        </ol>
      </section>
